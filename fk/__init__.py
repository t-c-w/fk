...
class ElasticSearch:
    ...
    def scan_and_scroll(self, index):
        response = self.get(
            '%s/_search?search_type=scan&scroll=1m' % index,
            data=json.dumps({'query': {'match_all': {}}, 'size': 100}),
        )
        while True:
            response = self.get('_search/scroll?scroll=1m', data=response['_scroll_id'])
    ...
    def bulk_insert(self, index, bulk):
        return self.post(
            '_bulk',
            data=''.join(
                json.dumps({'create': {'_index': index, '_type': line['_type']}})
                + '\n'
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--mapping', help='Mapping file, starts with {"mappings"...', type=FileType('r')
    )
